...
class ConsultPatientInfoScreen extends StatefulWidget {
  const ConsultPatientInfoScreen({super.key});

  @override
  State<ConsultPatientInfoScreen> createState() =>
      _ConsultPatientInfoScreenState();
}

class _ConsultPatientInfoScreenState extends State<ConsultPatientInfoScreen> {
  bool _isForMe = true;
  ...
  // CURP es opcional — no bloquea el avance
  bool get _isCurpValid =>
      _curpController.text.isEmpty || _curpController.text.length >= 18;
  ...
  Widget _buildPersonalInfoForm() {
    final isReadOnly = _isForMe;

    final nameError = (_hasAttemptedSubmit && _nameController.text.isEmpty)
    ...
    final emailError = _hasAttemptedSubmit
        ? (_emailController.text.isEmpty
            ? 'Este campo es requerido'
            : (!_isEmailValid
                ? 'Revisa que tu correo tenga un formato válido'
                : null))
        : null;

    final phoneError = _hasAttemptedSubmit
        ? (_phoneController.text.isEmpty
            ? 'Este campo es requerido'
            : (!_isPhoneValid ? 'Ingresa un número de 10 dígitos' : null))
        : null;

    final curpError = (_hasAttemptedSubmit && !_isCurpValid)
        ? 'La CURP debe tener 18 caracteres.'
        : null;

    final genderError = (_hasAttemptedSubmit && _gender == null)
        ? 'Este campo es requerido'
        : null;
    final stateError = (_hasAttemptedSubmit && _birthState == null)
        ? 'Este campo es requerido'
        : null;
    ...
    return Column(
      ...
      children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'El paciente debe ser mayor de edad para agendar una consulta.',
                        style: AppTheme.body()
        ...
        // CURP opcional
        _buildTextField('CURP (opcional)', _curpController,
